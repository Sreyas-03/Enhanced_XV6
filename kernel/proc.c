...
// dumy handler
void dummy(){
    printf("ALARM TRIGGRED (Def): TRING TRING TRING !!\n");
}

int _is_alarm_set = 0;
int start_tick = -1;
int end_tick = -1;
void (*_handler_ptr)(void) = dummy;

// calling handler when its time
int alarm_checker(){
  if(_is_alarm_set && end_tick == sys_uptime() ){
      printf("<\n");
      (*_handler_ptr)();
      printf(">\n");
...
// release all locked proc to resume computation
int alarm_stop(){
  if(_is_alarm_set){
    // struct proc *p;
    // for (p = proc; p < &proc[NPROC]; p++)
